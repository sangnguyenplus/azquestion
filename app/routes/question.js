		...
		Question.findById(req.params.question_id).populate("userId").exec(function(err, question){
			...
			question.save(function(err,q){
				...
				var mailOptions = {
				    ...
				    //text: 'Hello world', // Nội dung thư dạng thường
				    html: '<p><strong>Chào '+question.userId.displayName+'.</strong></p> <p>Câu hỏi "<strong>'+question.title+'</strong>" của bạn đã được đăng. Hãy bắt đầu thảo luận về chủ đề bạn thắc mắc nhé.</p>'
				    // Nội dung dạng html
				};
