...
module.exports = function (app, passport) {
    ...
    app.delete('/api/user/delete/:user_id', function(req, res) {
        var id = req.params.user_id;
        if (id == req.user._id) {
            return res.send({'error_msg': 'Lỗi. Bạn không thể xóa chính mình.'});
        }
        else {
            ...
            Answer.remove({userId: id}, function(err, answers) {
                if (err) {
                    return res.send(err);
                }
            });
            ...
            //Xóa đánh giá
            Vote.remove({userId: id}, function(err, votes) {
                if (err) {
                    return res.send(err);
                }
            });
    ...
    app.post('/api/user/resetPassword', function(req, res) {
        User.findOne({ resetPasswordToken: req.body.token, resetPasswordExpires: { $gt: Date.now() } }, function(err, user) {
            if (!user) {
              return res.send({'error_msg':'Mã khôi phục mật khẩu không hợp lệ hoặc đã hết hạn. Vui lòng yêu cầu mã khác!'});
            }
            else {
                user.password             = user.generateHash(req.body.NewPassword);
