angular.module('UserCtrl',[])
        ...
        //Nếu người dùng đã đăng nhập thì k được phép vào trang đăng nhập hoặc đăng ký.
        /*if($location.path().substr(0, $location.path().length)=="/login" ||
          $location.path().substr(0, $location.path().length)=="/register"
            ...
            $http.get('/loggedin').success(function(data){
              if(data!=="0")
              {
                flash.error="Bạn thực sự đăng nhập rồi!";
                $location.path('/');
              }
            });
          }*/
                    ...
                    if(u.length>0){
                      if(u[0].status===0){
                        ...
                        .success(function(data){
                                 ...
                                 socket.emit('add user', $cookieStore.get('currentUser').displayName);
